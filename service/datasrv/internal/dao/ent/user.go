...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldLogin, user.FieldAvatarURL, user.FieldHTMLURL:
			values[i] = new(sql.NullString)
		case user.ForeignKeys[0]: // issue_assignees
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case user.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field issue_assignees", value)
			} else if value.Valid {
				_m.issue_assignees = new(int64)
				*_m.issue_assignees = int64(value.Int64)
			}
		default:
